...
def agc04_load_in_13(roller_id):
    rospy.sleep(1)
    AGC04Roller("load")
    AGC04Roller("load")
    rospy.sleep(1)
    ...
    while not ( rospy.is_shutdown() or flag ):
        ...
        rospy.loginfo("--------------------------------------------------")
        #if ((status_of_roller13 == "roller13_complete_unload")or(status_of_roller13 == "roller13_complete_load")) and (status_of_agc04_roller == "complete_load"):
        if (status_of_agc04_roller == "complete_load"):
            ...
            flag = False
    return
def agc04_load_out_13(roller_id):
    rospy.sleep(1)
    Roller(13, "load")
    Roller(13, "load")
    rospy.sleep(1)
    ...
    while not ( rospy.is_shutdown() or flag ):
        ...
        rospy.loginfo("Roller13 status     : %s " % status_of_roller13)
        rospy.loginfo("----------------------------------------------------")
        #if (status_of_roller13 == "roller13_complete_load") and (status_of_agc04_roller == "complete_unload"):
        if (status_of_agc04_roller == "complete_unload"):
            rospy.loginfo("Done Un-Loading.")
            rospy.sleep(1)
            flag = True
        else:
...
def agc04_load_in_14(roller_id):
    rospy.sleep(1)
    AGC04Roller("load")
    AGC04Roller("load")
    rospy.sleep(1)
    ...
    while not ( rospy.is_shutdown() or flag ):
        ...
        rospy.loginfo("--------------------------------------------------")
        #if (status_of_roller14 == "roller14_complete_load") and (status_of_agc04_roller == "complete_load"):
        if (status_of_agc04_roller == "complete_load"):
            rospy.loginfo("AGC04-Loading is Done.")
            rospy.sleep(1)
            flag = True
...
def agc04_load_out_14(roller_id):
    rospy.sleep(1)
    Roller(14, "load")
    Roller(14, "load")
    rospy.sleep(1)
...
def agc04_load_in_15(roller_id):
    rospy.sleep(1)
    AGC04Roller("load")
    AGC04Roller("load")
    rospy.sleep(1)
    ...
    while not ( rospy.is_shutdown() or flag ):
        rospy.loginfo("AGC04 roller status : %s " % status_of_agc04_roller)
        rospy.loginfo("Roller15 status     : %s " % status_of_roller15)
        rospy.loginfo("--------------------------------------------------")
        #if (status_of_roller15 == "roller15_complete_load") and (status_of_agc04_roller == "complete_load"):
        if (status_of_agc04_roller == "complete_load"):
            rospy.loginfo("AGC04-Loading is Done.")
            rospy.sleep(1)
            flag = True
...
def agc04_load_out_15(roller_id):
    rospy.sleep(1)
    Roller(15, "load")
    Roller(15, "load")
    rospy.sleep(1)
    ...
    while not ( rospy.is_shutdown() or flag ):
        ...
        rospy.loginfo("Roller15 status     : %s " % status_of_roller15)
        rospy.loginfo("----------------------------------------------------")
        #if (status_of_roller15 == "roller15_complete_load") and (status_of_agc04_roller == "complete_unload"):
        if (status_of_agc04_roller == "complete_unload"):
            rospy.loginfo("Done Un-Loading.")
            rospy.sleep(1)
            flag = True
...
class agc04_AUTO_START():
    def __init__(self):
        ...
        ###################### Subscribe from agc04_AUTORUN_SW_status ####################################
        rospy.Subscriber('agc04_AUTORUN_SW_status', String, self.received_agc04_AUTORUN_SW_status)
        ##################################################################################################
        ###################### Subscribe from agc04_AUTORUN_SW_status ####################################
        rospy.Subscriber('agc04_E_STOP_status', String, self.received_agc04_E_STOP_status)
        ##################################################################################################
        ###################### Subscribe from agc04_AUTORUN_SW_status ####################################
        rospy.Subscriber('agc04_MASTERON_SW_status', String, self.received_agc04_MASTERON_SW_status)
        ##################################################################################################
        rospy.loginfo("  agc04_AUTO_START  ")
        rospy.sleep(1)
        # Begin the cmd_vel publishing loop
        while not rospy.is_shutdown():
            rospy.sleep(0.1)
            # rospy.loginfo("status_of_agc04_AUTORUN_SW  : %s " % status_of_agc04_AUTORUN_SW)
            # rospy.loginfo("status_of_agc04_E_STOP      : %s " % status_of_agc04_E_STOP)
            # rospy.loginfo("status_of_agc04_MASTERON_SW : %s " % status_of_agc04_MASTERON_SW)

            if ((status_of_agc04_E_STOP == "E_STOP_OFF") and (status_of_agc04_MASTERON_SW == "MASTERON_SW_ON")):
                rospy.sleep(0.1)
                rospy.loginfo("--------------------------------------------")
                return
            # else:
            #     rospy.sleep(0.1)
            #     rospy.loginfo("  agc04_AUTO_STAR  ")



    def received_agc04_AUTORUN_SW_status(self, msg):
        self.received_agc04_AUTORUN_SW = msg.data
        global status_of_agc04_AUTORUN_SW
        status_of_agc04_AUTORUN_SW = self.received_agc04_AUTORUN_SW
        # rospy.loginfo("status_of_agc04_AUTORUN_SW : %s " % status_of_agc04_AUTORUN_SW) 

    def received_agc04_E_STOP_status(self, msg):
        self.received_agc04_E_STOP = msg.data
        global status_of_agc04_E_STOP
        status_of_agc04_E_STOP = self.received_agc04_E_STOP
        # rospy.loginfo("status_of_agc04_E_STOP     : %s " % status_of_agc04_E_STOP) 

    def received_agc04_MASTERON_SW_status(self, msg):
        self.received_agc04_MASTERON_SW = msg.data
        global status_of_agc04_MASTERON_SW
        status_of_agc04_MASTERON_SW = self.received_agc04_MASTERON_SW
        # rospy.loginfo("status_of_agc04_MASTERON_SW : %s " % status_of_agc04_MASTERON_SW) 
...
if __name__ == '__main__':
    ...
    try:
        ...
        agc04_buzzer_cmd_pub = rospy.Publisher("agc04_buzzer_cmd", String, queue_size=1)
        ###########################################################################
        agc04_LED_cmd_pub = rospy.Publisher("agc04_LED_cmd", String, queue_size=1)
        ###########################################################################
        ...
        rospy.sleep(1)
        agc04_LED_cmd_pub.publish("RUN5")
        ...
        rospy.sleep(1)
        agc04_AUTO_START()
        agc04_LED_cmd_pub.publish("RUN1")
        agc04_LED_cmd_pub.publish("RUN1")
        agc04_buzzer_cmd_pub.publish("ch0")
        agc04_buzzer_cmd_pub.publish("ch0")
        agc04_buzzer_cmd_pub.publish("ch0")
        ...
        while not rospy.is_shutdown():
            ...
            talker("wait_a_minute")
            talker("wait_a_minute")
            rospy.loginfo("status :-------AR 13---goto Roller AR 14----------- ")
            agc04_buzzer_cmd_pub.publish("ch1")
            agc04_buzzer_cmd_pub.publish("ch1")
            agc04_LED_cmd_pub.publish("RUN1")
            agc04_LED_cmd_pub.publish("RUN1")
            rospy.sleep(1)
            ARFollower(13, 0.90, 0.1, 10)

            GoToPose(-1.70,0.11,0.0)    #GoToPose Roller AR 14  GoToPose(-1.80,0.10,0.0)
            agc04_LED_cmd_pub.publish("RUN2")
            agc04_LED_cmd_pub.publish("RUN2")
            ARFollower(14, 0.50, 0.006, 16)
            rospy.loginfo("status :-------------END Roller 14------------------ ")
            agc04_buzzer_cmd_pub.publish("ch0")
            agc04_buzzer_cmd_pub.publish("ch0")
            agc04_LED_cmd_pub.publish("OFF_")
            agc04_LED_cmd_pub.publish("OFF_")
            agc04_LED_cmd_pub.publish("OFF_")
            talker("clear_to_go")
            talker("clear_to_go")
            agc04_load_out_14(14)
            delay_loop(5)
            agc04_load_in_14(14)
            delay_loop(5)
            ...
            talker("wait_a_minute")
            talker("wait_a_minute")
            rospy.loginfo("status :------AR 14----goto Roller AR 13----------- ")
            rospy.sleep(0.5)
            agc04_LED_cmd_pub.publish("RUN1")
            agc04_LED_cmd_pub.publish("RUN1")
            agc04_buzzer_cmd_pub.publish("ch1")
            agc04_buzzer_cmd_pub.publish("ch1")
            rospy.sleep(1)
            ARFollower(14, 1.0, 0.1, 1)

            GoToPose(-1.20,1.15,-180)   #GoToPose Roller AR 13
            agc04_LED_cmd_pub.publish("RUN2")
            agc04_LED_cmd_pub.publish("RUN2")
            ARFollower(13, 0.50, 0.006, 16)
            rospy.loginfo("status :-------------END Roller 13------------------")
            agc04_buzzer_cmd_pub.publish("ch0")
            agc04_buzzer_cmd_pub.publish("ch0")
            agc04_LED_cmd_pub.publish("OFF_")
            agc04_LED_cmd_pub.publish("OFF_")
            ...
            talker("wait_a_minute")
            talker("wait_a_minute")
            rospy.loginfo("status :----AR 13------goto Roller AR 15----------- ")
            agc04_buzzer_cmd_pub.publish("ch1")
            agc04_buzzer_cmd_pub.publish("ch1")
            agc04_LED_cmd_pub.publish("RUN1")
            agc04_LED_cmd_pub.publish("RUN1")
            rospy.sleep(1)
            ARFollower(13, 0.90, 0.1, 1)

            GoToPose(-1.80,0.60,0.0)    #GoToPose Roller AR 15
            agc04_LED_cmd_pub.publish("RUN2")
            agc04_LED_cmd_pub.publish("RUN2")
            ARFollower(15, 0.5, 0.006, 16)
            rospy.loginfo("status :-------------END Roller 15------------------")
            agc04_buzzer_cmd_pub.publish("ch0")
            agc04_buzzer_cmd_pub.publish("ch0")
            agc04_LED_cmd_pub.publish("OFF_")
            agc04_LED_cmd_pub.publish("OFF_")
            ...
            talker("wait_a_minute")
            talker("wait_a_minute")
            rospy.loginfo("status :-----AR 15-----goto Roller AR 13----------- ")
            agc04_LED_cmd_pub.publish("RUN1")
            agc04_LED_cmd_pub.publish("RUN1")
            agc04_buzzer_cmd_pub.publish("ch1")
            agc04_buzzer_cmd_pub.publish("ch1")
            rospy.sleep(1)
            ARFollower(15, 1.0, 0.1, 1)

            GoToPose(-1.20,1.15,-180)   #GoToPose Roller AR 13  GoToPose(-1.20,1.15,-180)
            agc04_LED_cmd_pub.publish("RUN2")
            agc04_LED_cmd_pub.publish("RUN2")
            ARFollower(13, 0.50, 0.006, 16)
            rospy.loginfo("status :-------------END Roller 13------------------")
