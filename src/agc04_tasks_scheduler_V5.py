...
class ARFollower():
    ...
    def __init__(self, tag_id ,x_goal, y_goal, time_goal):
        ...
        self.max_angular_speed = rospy.get_param("~max_angular_speed", 0.091)  #0.1
        
        # The minimum rotation speed in radians per second
        self.min_angular_speed = rospy.get_param("~min_angular_speed", 0.055)  #0.1
...
if __name__ == '__main__':
    ...
    try:
        ...
        while not rospy.is_shutdown():
            ...
            agc04_buzzer_cmd_pub.publish("ch1")
            rospy.sleep(1)
            ARFollower(14, 1.0, 0.1, 10)
            GoToPose(0.85,2.5,90)   #GoToPose Roller AR 13


            agc04_LED_cmd_pub.publish("RUN2")
            agc04_LED_cmd_pub.publish("RUN2")
            ARFollower(13, 0.50, 0.04, 40)
...
#############################################################################################
            ...
            #--------------------------------------------------------------------#
            #--------------------------------------------------------------------#
            rospy.loginfo("status :----------goto Roller AR 15----------- ")
            agc04_buzzer_cmd_pub.publish("ch1")
            agc04_buzzer_cmd_pub.publish("ch1")
            agc04_LED_cmd_pub.publish("RUN1")
            agc04_LED_cmd_pub.publish("RUN1")
